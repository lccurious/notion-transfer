...
def image(info: dict) -> str:
    ...
    asset_path = Path(info["asset_root"].format(asset_type="img")) / info["file_path"]
    if info["caption"]:
        return (
            f'<div class="row">\n'
            f'    <div class="col-sm mt-3 mt-md-0">\n'
            f'        {{% include figure.liquid loading="eager" path="{asset_path}" title="{info["file_name"]}" class="img-fluid rounded z-depth-1" caption="{info["caption"]}" %}}\n'
            f'    </div>\n'
            f'</div>'
        )
    else:
        return (
            f'<div class="row">\n'
            f'    <div class="col-sm mt-3 mt-md-0">\n'
            f'        {{% include figure.liquid loading="eager" path="{asset_path}" title="{info["file_name"]}" class="img-fluid rounded z-depth-1" %}}\n'
            f'    </div>\n'
            f'</div>'
        )
